        ...
        $scope.message = 'Welcome Inspector ' + sessionStorage.getItem('inspector') ;
         ...
         $scope.init = function (url) {
                  ...
                  if(sessionStorage.getItem('inspector') === 'undefined'  || sessionStorage.getItem('inspector') === null )
                  { 
                  alert("Hello Assess: " + sessionStorage.getItem('inspector'));
                        $scope.Iname = 'Please login';
                        window.location = "" + url;
                  }    
                  else
        ...
        $scope.submit = function(url) 
      {
          ...
          $scope.message = 'Enter all required fields';
           ...
           else 
           {
            ...
            $http.post(' http://testserver.comyr.com/assess.php', {'utemp': $scope.temp, 'urh': $scope.rh, 'uprep': $scope.prep }
                        ).success(function(data, status, headers, config) {
                            window.location = "" + url;
     ...
     HealthyHomes.controller('buildingCharB1B2B3', function ($scope, $http, $log, $timeout,$filter) {
        ...
        $scope.prevPage = function(url) {
         ...
         $scope.change = function() {
                $scope.year_built = 2014;
                $scope.year_renovated = 2014;
                $scope.desc = 'NA';
            };
        ...
        {
            $scope.flag = false;
            if($scope.built === "1950-1978" && $scope.year_built > 1950 && $scope.year_built < 1978 )
            {
                $scope.flag = true;
            }
            else
                if($scope.built === "after 1978" && $scope.year_built > 1978 && $scope.year_built < 2014)
                {
                        $scope.flag = true;
                }
                else
                    if($scope.built === "before 1950" && $scope.year_built < 1950)
                    {
                        $scope.flag = true;
                    }
        ...
        else
        {
            ...
            if($scope.year_renovated > $scope.year_built)
            {
            $scope.flag1 = true;
            }
            ...
            $scope.next = function(url) 
            {
              ...
              // If form is invalid, return and let AngularJS show validation errors.
              if ($scope.b1b2b3.$invalid || !$scope.flag || !$scope.flag1) {
          ...
          $scope.prevPage = function(url) {
            window.location = url + "";
                ...
                $scope.roomSlected = function(option,url){
                    ...
                    {
                        case 1:
                            var count = (sessionStorage.getItem('livingRoom') === null) ? 0 : parseInt(sessionStorage.getItem('livingRoom'), 0);
                            count = count + 1;
                            sessionStorage.setItem('livingRoom',count);
                            sessionStorage.setItem('RoomType','livingRoom');
                            break;
                        case 2:
                            var count1 = (sessionStorage.getItem('bedRoom') === null) ? 0 : parseInt(sessionStorage.getItem('bedRoom'),0);
                            count1 = count1 + 1;
                            sessionStorage.setItem('bedRoom',count1);
                            sessionStorage.setItem('RoomType','bedRoom');
                            break;
                        case 3:
                            var count2 = (sessionStorage.getItem('kitchen') === null) ? 0 : parseInt(sessionStorage.getItem('kitchen'), 0);
                            count2 = count2 + 1;
